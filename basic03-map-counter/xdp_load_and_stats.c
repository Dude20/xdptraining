			...
			struct stats_record *stats_prev)
{
	...
	double period;
	__u64 packets, bytes;
	double pps, mbits; /* packets per sec */ /* mbits per sec */
	
	/* Assignment#2: Print other XDP actions stats  */
	__u32 xdp_action_counter;
	for(xdp_action_counter=0; xdp_action_counter<XDP_ACTION_MAX; xdp_action_counter++)
	{
		char *fmt = "%-12s %'11lld pkts (%'10.0f pps)"
			" %'11lld Kbytes (%'6.0f Mbits/s)"
			" period:%f\n";
		const char *action = action2str(xdp_action_counter);
		//const char *action = action2str(XDP_PASS);
		rec  = &stats_rec->stats[xdp_action_counter];
		prev = &stats_prev->stats[xdp_action_counter];
			  ...
			  struct stats_record *stats_rec)
{
	...
	//__u32 key = XDP_PASS;
	__u32 action_counter;
	for(action_counter = 0;  action_counter<XDP_ACTION_MAX;action_counter++)
		map_collect(map_fd, map_type, action_counter, &stats_rec->stats[action_counter]);
}
